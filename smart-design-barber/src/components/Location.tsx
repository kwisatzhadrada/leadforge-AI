...
import { business, hours, hoursFootnote } from "../data/business";
      ...
      <div className="wrap">
        <div ref={headRef}>
          <SectionHead
            ...
            title={
              <>
                Find Us On <span className="brass-text">Stroud Green Road</span>
              </>
            }
          />
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
          <div ref={infoRef} className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-8 md:p-9">
            ...
            <Row icon="phone" title="Phone">
              <a href={`tel:${business.phone.tel}`} className="hover:text-[var(--brass)]">
            ...
            <Row icon="clock" title="Hours">
              ...
              <p className="mt-3 text-[0.76rem] text-[var(--text-faint)]">{hoursFootnote}</p>
            </Row>
            ...
            <div className="mt-2 flex flex-wrap gap-3">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-[#150f06] bg-[linear-gradient(135deg,var(--brass-bright),var(--brass))]"
              >
                Directions <Icon name="arrowRight" className="h-[18px] w-[18px]" />
              </a>
              <a
                href={`tel:${business.phone.tel}`}
                className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] px-6 py-3 text-sm font-semibold hover:border-[var(--brass)]"
              >
            ...
            <div className="mt-6 flex gap-2.5 border-t border-[var(--border-soft)] pt-6 text-[0.82rem] text-[var(--text-faint)]">
              <Icon name="pin" className="h-4 w-4 shrink-0 text-[var(--steel)]" />
              On Stroud Green Road in Finsbury Park — an easy walk from Finsbury Park station.
            </div>
          </div>

          <div ref={mapRef} className="min-h-[340px] overflow-hidden rounded-[22px] border border-[var(--border)]">
            <iframe
              title="Smart Design Barber location map"
...
function Row({ icon, title, children }: { icon: string; title: string; children: ReactNode }) {
  return (
    <div className="mb-6 flex gap-3.5">
      <Icon name={icon as never} className="mt-0.5 h-5 w-5 shrink-0 text-[var(--brass)]" />
