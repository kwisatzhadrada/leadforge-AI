...
const credits: Credit[] = [
  ...
  { label: "Heritage", ...aboutImage },
  { label: "Experience", ...experienceImage },
  ...galleryImages.map((g) => ({ label: g.tag, alt: g.alt, sourceUrl: g.sourceUrl, photographer: g.photographer })),
];

export function Footer() {
  return (
    <footer className="border-t border-[var(--border-soft)] bg-[var(--bg-elevated)] pb-8 pt-16">
      <div className="wrap">
        <div className="mb-12 grid gap-10 sm:grid-cols-[1.4fr_1fr_1fr]">
          <div>
            <a href="#top" className="flex items-center gap-2.5 text-[1.1rem]" style={{ fontFamily: "var(--font-display)" }}>
            ...
            <p className="mt-3.5 max-w-[320px] text-[0.9rem] text-[var(--text-dim)]">
              Turkish-style barbering on Stroud Green Road, Finsbury Park, since {business.since}.
            </p>
            <div className="mt-4 flex gap-2.5">
              {(["instagram", "facebook", "tiktok"] as const).map((s) => (
                <a
                  ...
                  href="#"
                  aria-label={s}
                  className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--border)] text-[var(--text-dim)] transition-colors hover:border-[var(--brass)] hover:text-[var(--brass)]"
                >
        ...
        <details className="mb-8 rounded-[14px] border border-[var(--border-soft)] p-5 text-[0.82rem] text-[var(--text-faint)]">
          ...
          <p className="mt-4">
            Stand-in photography, not the actual shop or staff — swap for real Smart Design
            Barber photos when available.
          </p>
        </details>
