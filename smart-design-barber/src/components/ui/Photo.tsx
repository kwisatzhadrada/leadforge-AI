...
export function Photo({ src, alt, icon = "photo", className = "", loading = "lazy", style }: Props) {
  ...
  if (errored) {
    return (
      <div
        className={`relative flex h-full w-full flex-col items-center justify-center gap-2.5 overflow-hidden bg-[linear-gradient(155deg,var(--surface-2)_0%,var(--bg-elevated)_65%,var(--surface)_100%)] ${className}`}
      >
        <div
          className="pointer-events-none absolute inset-[-20%] opacity-40"
          style={{
            backgroundImage:
              "repeating-linear-gradient(135deg, rgba(242,236,224,0.04) 0 2px, transparent 2px 26px)",
          }}
        />
        <Icon name={icon as never} className="relative h-8 w-8 text-[var(--brass-dark)] opacity-70" />
